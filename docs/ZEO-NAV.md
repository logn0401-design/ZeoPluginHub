...
**v0.1.23 — Precision Attitude (Pulsar distribution)**

Install **Zeo Nav (Pulsar Test)** from the [Zeo Plugins catalog](../README.md#add-this-catalog). Requires Pulsar 2.4.2 or later, Legacy, Windows, and Space Engineers 1. The matching external HUD downloads automatically; a separate Nav installer or .NET SDK is not needed for players.

## First installation

1. Add the Zeo Plugins remote hub using the repository README, then refresh sources.
2. Enable **Zeo Nav (Pulsar Test)** and fully restart the game.
3. In Pulsar, use Nav's **Open Config** action. Select a GPS destination, your speed limit and MAX SIG, then press GO.
...
This is the v0.1.23 precision-attitude baseline with a Pulsar loader and automatic overlay delivery. It includes the compact Zeo-style native menu, 750 KM MAX SIG, 50,000 m/s speed settings, automatic SDX/Epstein main-drive classification, server thrust handling, and near-target angular-velocity damping. No further flight-control tuning is part of this catalog conversion.

## Updating and checking

Refresh Pulsar sources and fully restart. The Nav log in `%APPDATA%/Pulsar/ZeoNav/zeonav.log` should identify `0.1.23-PRECISION-ATTITUDE-PULSAR`. The descriptor pins an immutable source commit and SHA-256 hashes for both runtime assets.

Offline checks are documented in [Nav validation](ZEO-NAV-VALIDATION.md). A clean catalog installation and flight on a mate's PC still require an in-game test. This release does not claim that every server's wobble has been eliminated.
...
Commit source/assets first, then pin that full commit in `Plugins/ZeoNav.xml` and update asset hashes in a second commit. Preserve Core and other catalog entries when publishing. The existing one-click v0.1.23 package remains the manual fallback.
